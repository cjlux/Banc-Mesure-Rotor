...
class ROTOR_bench:
    ...
    def config_USBsensor(self):
        if self.serialPort is None:
            ...
            return -1
        self.serialPort.write(f'NS {Param.SENSOR_NB_SAMPLE.value}'.encode('ascii'))
        self.serialPort.write(f'PG {Param.SENSOR_GAIN.value}'.encode('ascii'))
        return 0
    ...
    def write_header(self, file_name:str, rot_step:float = None, NBSTEP1:int = None, Zpos:list = None):
        ...
        with open(file_name, "w", encoding="utf8") as fOut:
            ...
            for p in Param:
                if'SENSOR' in p.name: fOut.write(f'# {p.name}: {p.value} \n')
    ...
    def Zref_sensor(self, hold_torque:bool = False, verbose:bool = False):
        ...
        N_Hz  = 2 * Zaxis.Zref_velocity.value / (np.pi * Stepper2.DIAM_MM
                                                 )
        T_sec = 1 / (N_Hz * Stepper2.NB_STEP_PER_REVOL)
    ...
    def continuous_reccord(self, duration=10, SENSOR_READ_DELAY=None):
        ...
        if SENSOR_READ_DELAY is None:
            SENSOR_READ_DELAY = Param.SENSOR_READ_DELAY.value
        ...
        with open(fileName, "w", encoding="utf8") as fOut:
            ...
            while True:
                ...
                X, Y, Z = map(float, data.split(','))
                X *= Param.SENSOR_Oe_mT.value
                Y *= Param.SENSOR_Oe_mT.value
                Z *= Param.SENSOR_Oe_mT.value
